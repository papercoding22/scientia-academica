---
name: new-assignment
description: Tạo đầy đủ một bài tập mới — chạy script dựng aN/ và copy sẵn file nộp từ template Word đúng mẫu tên giảng viên, rồi trích đề bài từ transcript buổi học, ghi hạn nộp vào admin/deadlines.md và cập nhật bảng bài tập của môn. Dùng khi người dùng nói "tạo bài tập N", "có bài tập mới", "thầy giao bài", "init assignment", hoặc đưa đề bài của một bài tập chưa có thư mục trong repo.
---
...
|---|---|
| Mã môn | **Hỏi.** |
| Số bài tập (`8`, `3A`) | **Hỏi.** Không suy từ số thư mục đang có — số bài hay nhảy cóc |
...
> ⚠️ **Số bài bám theo cách giảng viên đánh số, không phải thứ tự thư mục.**
...
Script tạo `aN/` với `README.md`, `brief/`, `resources/`, `images/`, và **copy sẵn
`templates/ASSIGNMENT_TEMPLATE.docx` thành file nộp đúng mẫu tên giảng viên**.

**Mẫu tên file nộp được SUY RA từ bài đã nộp trong cùng môn** — script không hardcode,
nó bắt chước đúng file trước đó. Môn chưa có bài nào thì script cảnh báo và bỏ qua
...
Tìm được thì điền vào mục **Yêu cầu đề bài** của `aN/README.md`, **kèm nguồn**:
...
| `<môn>/README.md` | Dòng vào bảng **Bài tập và đồ án**: link `aN/` · tên · buổi · hạn · trạng thái |
...
- ❌ **Không tự đổi mẫu tên file nộp.** Mẫu do giảng viên quy định (`AGENTS.md` § 13.3).
  Script suy ra từ bài trước — nếu thấy nó sai, hỏi người dùng, đừng "sửa cho đẹp".
